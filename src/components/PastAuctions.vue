<template>
  <b-container class="table-container">
    ...
    <DataTableV2 :fields="auctions.fields" class="px-0" :pagination="auctions.pagination" :loadItems="loadPast">
      <!-- column: height_range -->
...
export default {
  ...
  methods: {
    async loadPast(network, page, limit) {
      const { auctions, totalRows } = await this.$api.auction.getPast(network, page, limit);
